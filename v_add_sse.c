...
static inline float *
vec_addNf_sse_lt16(float *va, const float *vb, unsigned int ne)
{
  __m128 mva1;
  __m128 mvb1;
  const float *pvb;
...
static inline float *
vec_addNf_sse_gte16(float *va, const float *vb, unsigned int ne)
{
  __m128 mva1;
  __m128 mva2;
  __m128 mva3;
...
{
  return (ne >= 16) ? vec_addNf_sse_gte16(va, vb, ne)
                    : vec_addNf_sse_lt16(va, vb, ne);
}
