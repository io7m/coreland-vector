...
vec_addscNd_sse2(double *va, double sc, unsigned int ne)
{
  ...
  for (ind = 0; ind < seg[1]; ++ind) {
    ...
    mva1 = _mm_add_pd(mva1, vsc);
    mva2 = _mm_add_pd(mva2, vsc);
...
static inline double *
vec_addscNdx_sse2(const double *va, double *vr, double sc, unsigned int ne)
{
  __m128d mva1;
  __m128d mva2;
  __m128d mvr;
  ...
  for (ind = 0; ind < seg[1]; ++ind) {
    mva1 = _mm_load_pd(pva);
    mva2 = _mm_load_pd(pva + 2);
    mvr = _mm_add_pd(mva1, vsc);
    _mm_store_pd(pvr, mvr);
    mvr = _mm_add_pd(mva2, vsc);
