...
vec_subNf_sse_gte16(float *va, const float *vb, unsigned int ne)
{
  ...
  if (nr) vec_subNf_sse_lt16(pva, pvb, nr);
  return va;
}
...
static inline float *
vec_subNfx_sse_lt16(const float *va, const float *vb, float *vr, unsigned int ne)
{
  __m128 mva1;
  __m128 mvb1;
  const float *pva;
...
static inline float *
vec_subNfx_sse_gte16(const float *va, const float *vb, float *vr, unsigned int ne)
{
  __m128 mva1;
  __m128 mva2;
  __m128 mva3;
  ...
  if (nr) vec_subNfx_sse_lt16(pva, pvb, pvr, nr);
  return vr;
}
...
{
  return (ne >= 16) ? vec_subNfx_sse_gte16(va, vb, vr, ne)
                    : vec_subNfx_sse_lt16(va, vb, vr, ne);
}
