...
have()
{
  grep "$1" _sysinfo.h 1>/dev/null 2>&1
}
...
if have SYS_HAVE_CPU_EXT_SSE
then
  cat <<EOF
/* SYS_HAVE_CPU_EXT_SSE */
#include <xmmintrin.h>
...
if have SYS_HAVE_CPU_EXT_ALTIVEC
then
  cat <<EOF
/* SYS_HAVE_CPU_EXT_ALTIVEC */
...
if have SYS_HAVE_CPU_EXT_SSE2
then
  cat <<EOF
/* SYS_HAVE_CPU_EXT_SSE2 */
#include <emmintrin.h>
...
if ! have SYS_HAVE_CPU_EXT_SSE && ! have SYS_HAVE_CPU_EXT_ALTIVEC
then
  cat <<EOF
/* ! SYS_HAVE_CPU_EXT_SSE && ! SYS_HAVE_CPU_EXT_ALTIVEC */
