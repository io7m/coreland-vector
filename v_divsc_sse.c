...
static inline float *
vec_divscNf_sse_gte16(float *va, float sc, unsigned int ne)
{
  __m128 mva1;
  __m128 mva2;
  __m128 mva3;
  ...
  for (ind = 0; ind < n16; ++ind) {
    ...
    mva3 = _mm_div_ps(mva3, vsc);
    mva4 = _mm_div_ps(mva4, vsc);
    _mm_store_ps(pva, mva1);
  ...
  if (nr) vec_divscNf_sse_lt16(pva, sc, nr);
  return va;
}

static inline float *
vec_divscNf_sse(float *va, float sc, unsigned int ne)
{
  return (ne >= 16) ? vec_divscNf_sse_gte16(va, sc, ne)
                    : vec_divscNf_sse_lt16(va, sc, ne);
}

static inline float *
vec_divscNfx_sse(const float *va, float *vr, float sc, unsigned int ne)
{
  __m128 mva1;
  __m128 vsc;
  const float *pva;
